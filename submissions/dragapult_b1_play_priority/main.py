    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            if card.id == Dreepy:
                ...
                if card_score > 0:
                    # Arm B1. The empty-bench branch in hand_score only exists to make this card
                    # *legal* to play as a last resort (without it the card_score > 0 gate above
                    # vetoes it even onto a fully empty bench). At 53000 it instead outranks
                    # Dreepy's 51000 and Budew's 52000, so the agent benches a 2-prize ex ahead of
                    # its own evolution engine. Drop that specific case below Dreepy; leave the
                    # pre-KO ability-timing case, which genuinely wants top priority, at 53000.
                    score = 50500 if len(my_state.bench) == 0 else 53000
                else:
                    score = -1
