...
if not os.path.exists(file_path):
    # Real Kaggle sandbox path (main.py runs there via exec(), no __file__ available).
    file_path = "/kaggle_simulations/agent/deck.csv"
if not os.path.exists(file_path):
    # Local test harness only: __file__ IS available here (loaded via importlib),
    # guarded so it can never run in the real exec()-without-__file__ sandbox above.
    try:
