    ...
    def hand_score(id: int, ignore_count: bool):
        ...
        if id == Dreepy:
            ...
            if pre_ko:
                ...
                # Arm B2. 25000 was picked only to be "clearly nonzero" so the OptionType.PLAY
                # gate (card_score > 0) would stop vetoing this card onto an empty bench. But
                # hand_score has three other consumers where the magnitude is load-bearing:
                # TO_BENCH/TO_HAND use it as the priority directly (25000 outranks Dreepy's
                # 18000), DISCARD uses -hand_score (25000 makes it the last card ever discarded),
                # and the Night_Stretcher branch gates on card_score >= 18000 (25000 trips it).
                # 3000 keeps the PLAY gate open while sitting below all three thresholds — the
                # same rank as a Drakloak with nothing to evolve from.
                score = 3000
        elif id == Latias_ex:
